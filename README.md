# 全体のアーキテクチャー
3-Lowアーキテクチャーのフロントエンドに該当する構成例についてCopilot Studioで利用可能な Tips を紹介します。

![3LA](images/00.png)

# ユーザー選択に基づく会話フロー
チャットによるユーザーのガイド、場合によっては決まった質問に対しては固定の回答を応答させたい場合があります。
</br>
また、ユーザーからのフィードバックを元に別のアクションを呼び出しチャットエクスペリエンスを向上させたい場合もあるかもしれません。
</br>
それぞれのシナリオに合わせてカスタマイズ例を紹介します。

## Adaptive Card を利用し、ユーザーに選択肢を提示する。
[Adaptive Card をノードに追加する](https://learn.microsoft.com/ja-jp/microsoft-copilot-studio/authoring-ask-with-adaptive-card#add-an-adaptive-card-node)を参照し、[Adaptive Card Designer](https://adaptivecards.io/designer/) で生成可能な JSON を貼り付けます。
...
```
Copilot Studio トピック編集画面に以下のようなプレビューが表示されれば、作業完了です。

![AdaptiveCard1](images/01.png)

## Adaptive Card を利用し、ユーザーからフィードバックを収集する。
[Adaptive Card をノードに追加する](https://learn.microsoft.com/ja-jp/microsoft-copilot-studio/authoring-ask-with-adaptive-card#add-an-adaptive-card-node)を参照し、[Adaptive Card Designer](https://adaptivecards.io/designer/) で生成可能な JSON を貼り付けます。
